...
- 以threejs为例，matrix为列存储，矩阵set及计算方式详细分析如下
 * set方法 行优先，内部存储array 列优先，所以内部会转一次
...
- vector.apply(matrix)实际是矩阵与向量相乘，向量是4x1矩阵，matrix是4x4矩阵，matrix与vector相乘是4x4 · 4x1，结果是4x1矩阵，即vector.
```javascript
const v = new Vector3();
const m = new Matrix4();
